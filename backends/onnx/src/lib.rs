...
impl Backend for OnnxBackend {
    ...
    fn capabilities(&self) -> BackendCapabilities {
        BackendCapabilities {
            ...
            accelerators: vec!["onnx".to_owned()],
            hardware: Some("onnx".to_owned()),
            notes: vec!["Architecture boundary established for onnx integration".to_owned()],
        }
    }
    ...
    async fn infer_stream(
        ...
        Ok(Box::pin(stream::iter(vec![Err(
            RuntimeError::CapabilityMismatch {
                reason: "streaming is not available for the onnx scaffold backend".to_owned(),
            },
        )])))
    }
}
