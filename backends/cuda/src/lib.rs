...
impl Backend for CudaBackend {
    ...
    fn capabilities(&self) -> BackendCapabilities {
        BackendCapabilities {
            ...
            notes: vec!["Architecture boundary established for cuda integration".to_owned()],
        }
    }
    ...
    async fn infer_stream(
        ...
        Ok(Box::pin(stream::iter(vec![Err(
            RuntimeError::CapabilityMismatch {
                reason: "streaming is not available for the cuda scaffold backend".to_owned(),
            },
        )])))
    }
}
...
mod tests {
    ...
    fn descriptor_matches_boundary() {
        assert_eq!(CudaBackend::descriptor().name, "cuda");
    }
}
